...
5. 测试、Lint、服务端渲染检查和生产构建全部通过；
6. 所有旧远端分支均为最新 `main` 的祖先，删除不会丢失提交。
...
`package.json` 使用 `panlayer` 作为包名，补充描述、仓库、主页和 MIT 许可证字段，并保留 `private: true`，避免误发布到 npm；该字段不影响 GitHub 开源。
