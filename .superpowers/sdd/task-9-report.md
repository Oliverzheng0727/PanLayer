...
- Centralized retry and failure diagnostics in a shared sanitizer. It redacts secret assignments and credentials, strips control characters, neutralizes prompt-boundary characters, and is JSON-escaped in retry prompts.
- Rejected malformed snapshot numerics in both single-label and ordered `分别` clauses using strict decimal and grouped-thousands grammar; ranking-token diagnostics no longer echo model-authored prose.
