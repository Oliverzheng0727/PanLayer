...
export function HistoryWorkspace({ initialRows = [] }: { initialRows?: HistoryRow[] }) {
  ...
  return (
    <div className="history-workspace panel">
      ...
      <div className="history-layout">
        ...
        <HistoryTable rows={visible} selected={selected} sort={sort} order={order} onSelect={selectDate} onSort={cycleSort} onNearEnd={() => setVisibleCount((count) => Math.min(sorted.length, count + 10))} />
      </div>
    </div>
  );
}
